...
from utils.exporters import gerar_csv_grupos, gerar_excel_grupos, gerar_txt_grupos
from utils.qr_generator import gerar_qr_code_grupo, gerar_qr_code_todos_grupos
from utils.helpers import formato_display
from ui.components import card_estatistica, alerta_info
...
def exibir_grupos(grupos, tamanho_grupo, estudantes_originais, show_animation=True):
    ...
    with st.expander("📱 QR Codes"):
        ...
        if qr_option == "Por Grupo":
            ...
            if st.button("🎯 Gerar QR Code do Grupo"):
                ...
                st.markdown(f"<small>Contém dados JSON com {len(grupos[qr_grupo_num - 1])} estudantes</small>", unsafe_allow_html=True)
            ...
            if st.button("🎯 Gerar QR Code de Todos os Grupos"):
                ...
                st.markdown(f"**QR Code - Todos os Grupos**")
                st.markdown(f"<small>Contém dados JSON com {len(grupos)} grupos</small>", unsafe_allow_html=True)
                ...
                st.download_button(
                    "⬇️ Download QR Code",
                    data=img_bytes,
                    file_name=f"qr_todos_grupos.png",
                    mime="image/png"
                )
    ...
    if stats["menor_grupo"] < tamanho_grupo:
        alerta_info(f"Alguns grupos têm menos estudantes ({stats['menor_grupo']}) porque o número total não é divisível por {tamanho_grupo}.")
    
    if stats["maior_grupo"] > tamanho_grupo:
        alerta_info(f"Alguns grupos têm mais estudantes ({stats['maior_grupo']}) para evitar que alunos fiquem sozinhos.")
...
def exibir_grupo_individual(grupo, numero, formato_exibicao):
    ...
    with col2:
        # QR Code individual
        if st.button(f"📱 QR Code", key=f"qr_{numero}", use_container_width=True):
            import base64
